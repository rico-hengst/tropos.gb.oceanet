...
if not os.path.isfile( cfg_instruments_file ):
    print( 'File not exists '+ cfg_instruments_file )
    quit()
if not os.path.isfile( cfg_missions_file ):
    print( 'File not exists '+ cfg_missions_file )
    quit()
...
for mission in cfg_missions:
    ...
    if not ( cfg_missions.get(mission,{}).get("datetime_start") and cfg_missions.get(mission,{}).get("datetime_stopp") ):
        print("start stopp date of mission not exists??")
        quit()
...
def get_instruments( mission_of_interest ):
    print("Start to collect related instruments to mission of interest: " + mission_of_interest )
    print("Show metadata starting from selected mission -> get instruments")
    print("------------------------------------------------------")
    
    # check if mission_of_interest exists
    if not cfg_missions.get(mission_of_interest, {}):
        print("Error, mission not exists: " + mission_of_interest)
        quit()
...
def get_missions( instrument_of_interest ):
    print("Start to collect related missions to instrument of interest: " + instrument_of_interest )
    print("Show metadata starting from selected instrument -> get missions")
    print("------------------------------------------------------")

    # check if instrument_of_interest exists
    if not cfg_instruments.get(instrument_of_interest, {}):
        print("Error, instrument not exists: " + instrument_of_interest)
        quit()
